...
import com.nbastat.player.generators.domain.factory.NBAStatEndpointsDefinitionFactory;
...
public class NBAStatEndpointsDefinitionFactoryTests {
	...
	public void buildFromFileNameHappyPath() {
		...
		NBAStatHeaders headers = NBAStatHeadersBuilder.builder()
				...
				.withReferer("http://stats.nba.com/players/t…17&SeasonType=Regular%20Season")
				.withUserAgent("Mozilla/5.0 (X11; Fedora; Linu…) Gecko/20100101 Firefox/59.0")
		...
		endpoints.getRequiredParameters().add("MeasureType");
		endpoints.getRequiredParameters().add("PerMode");
		endpoints.getRequiredParameters().add("Season");
		endpoints.getRequiredParameters().add("SeasonType");
		endpoints.getParameters().add("College");
		endpoints.getParameters().add("Conference");
		endpoints.getParameters().add("Country");
		endpoints.getParameters().add("MeasureType");
		endpoints.getParameters().add("PerMode");
		endpoints.getParameters().add("Season");
		endpoints.getParameters().add("SeasonType");
		...
		dummyNbaStatsEndointsDefinition.getEndpoints().add(endpoints);
		dummyNbaStatsEndointsDefinition.getParameters().add(parameter);
		
		NBAStatEndpointsDefinitionFactory nbaStatEndpointsDefinitionFactory = new NBAStatEndpointsDefinitionFactory();
		
		NBAStatEndpointsDefinition nbaStatEndpointsDefinition = nbaStatEndpointsDefinitionFactory.buildFromJsonFile(testFileName);
		
		assertThat(nbaStatEndpointsDefinition.getHeaders()).isEqualTo(headers);
		
		assertThat(nbaStatEndpointsDefinition.getEndpoints().size()).isEqualTo(dummyNbaStatsEndointsDefinition.getEndpoints().size());
		for(int x=0; x < nbaStatEndpointsDefinition.getEndpoints().size(); x++) {
			assertThat(nbaStatEndpointsDefinition.getEndpoints().get(x)).isEqualTo(dummyNbaStatsEndointsDefinition.getEndpoints().get(x));
		}
